#ifndef flexPermutationOperator_H
#define flexPermutationOperator_H
...
template<typename T>
class flexPermutationOperator : public flexLinearOperator<T>
{
...
public:
	...
	*/
	flexPermutationOperator(int aNumRows, int aNumCols, std::vector<int> aIndices,  bool aMinus) : flexLinearOperator<T>(aNumRows, aNumCols, permuteOp, aMinus)
        , indices(aIndices)
    {
	...
	flexPermutationOperator<T>* copy()
	{
		flexPermutationOperator<T>* A = new flexPermutationOperator<T>(this->getNumRows(), this->getNumCols(), this->isMinus);
		return A;
	}
